...
class EmpsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create emp" do
    assert_difference('Emp.count') do
      post emps_url, params: { emp: { address: @emp.address, contact: @emp.contact, email: @emp.email, name: @emp.name } }
    end
  ...
  test "should update emp" do
    patch emp_url(@emp), params: { emp: { address: @emp.address, contact: @emp.contact, email: @emp.email, name: @emp.name } }
